...
class HuDataset(Dataset):
    ...
    def __init__(self, image_id, image_dir, augment=None):
        ...
        tile_id = []
        for id in image_id:
            df = pd.read_csv(data_dir + '/tile/%s/%s.csv'% (self.image_dir,id) )
    ...
    def __getitem__(self, index):
        ...
        image = cv2.imread(data_dir + '/tile/%s/%s.png'%(self.image_dir,id), cv2.IMREAD_COLOR)
        mask  = cv2.imread(data_dir + '/tile/%s/%s.mask.png'%(self.image_dir,id), cv2.IMREAD_GRAYSCALE)
...
def run_check_augment():
    def augment(image, mask):
        ...
        #image, mask = do_random_hsv(image, mask, mag=[0.1,0.2,0.2])
        #image, mask = do_random_noise(image, mask, mag=0.1)

        image, mask = np.random.choice([
            lambda image, mask: do_random_rotate_crop(image, mask, size=320, mag=30),
            lambda image, mask: do_random_scale_crop(image, mask, size=320, mag=0.05),
            lambda image, mask: do_random_crop(image, mask, size=320),
        ])(image, mask)

        image, mask = np.random.choice([
            lambda image, mask: (image, mask),
            lambda image, mask: do_random_contast(image, mask, mag=0.2),
            lambda image, mask: do_random_hsv(image, mask, mag=[0.05, 0.2, 0.2]),
    ...
    #for i in range(1000):
    for i in np.random.choice(len(dataset),1000000000):
        r = dataset[i]
        image = r['image']
        mask  = r['mask']
