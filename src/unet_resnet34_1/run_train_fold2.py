...
def train_augment(record):
    ...
    image, mask = random.choice([
        lambda image, mask : do_random_rotate_crop(image, mask, size=320, mag=20),
...
def run_train():
    ...
    fold = 2
    out_dir = '/root/share1/kaggle/2020/hubmap/result/resnet34/fold%d'%fold
    initial_checkpoint = \
        out_dir+'/checkpoint/00010000_model.pth' #
        #'/root/share1/kaggle/2020/hubmap/result/resnet34/fold2/checkpoint/00003000_model.pth'
    ...
    train_id = make_image_id ('train-%d'%fold)
    valid_id = make_image_id ('valid-%d'%fold)
